...
function realizarRetiro($id_usuario, $saldoDisponible, $montoRetiro, $tipo, $db)
{
  if ($montoRetiro > 0 && $montoRetiro <= $saldoDisponible) {
    ...
    try {
      ...
      $registrarTransaccion = $db->prepare("
                INSERT INTO transaccion (fk_usuario, fecha, tipo, monto)
                VALUES (:fk_usuario, NOW(), :tipo, :monto)
            ");
      $registrarTransaccion->bindParam(':fk_usuario', $id_usuario);
      $registrarTransaccion->bindParam(':tipo', $tipo);
...
function realizarTransferencia($id_usuario, $saldoDisponible, $idUsuarioDestinatario, $montoTransferencia, $db)
{
  if ($montoTransferencia > 0) {
    ...
    try {
      $actualizarSaldoOrigen = $db->prepare("UPDATE cuenta SET saldo = :nuevoSaldo WHERE fk_usuario = :idUsuario");
      ...
      $actualizarSaldoDestinatario = $db->prepare("UPDATE cuenta SET saldo = :nuevoSaldo WHERE fk_usuario = :idUsuario");
      ...
      //Registrar transacción para el usuario que la recibe
      $registrarTransaccionRecibida = $db->prepare("
                INSERT INTO transaccion (fk_usuario, fecha, tipo, monto)
                VALUES (:fk_usuario, NOW(), 'transferencia recibida', :monto)
            ");
      $registrarTransaccionRecibida->bindParam(':fk_usuario', $idUsuarioDestinatario);
      $registrarTransaccionRecibida->bindParam(':monto', $montoTransferencia);
      $registrarTransaccionRecibida->execute();
      $db->commit();
